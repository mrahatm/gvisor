...
func (t *thread) init() {
	// Set our TRACESYSGOOD option to differeniate real SIGTRAP. We also set
	// PTRACE_O_EXITKILL to ensure that the unexpected exit of the sentry will
	// immediately kill the associated stubs.
	...
	_, _, errno := syscall.RawSyscall6(
		...
		uintptr(t.tid),
		0,
		syscall.PTRACE_O_TRACESYSGOOD|PTRACE_O_EXITKILL,
		0, 0)
	if errno != 0 {
